<?php
    ...
    if (isset($_POST['sbmReviewBtn'])){
        ...
        if ($movie_id) {
            ...
            if(mysqli_num_rows($validateResult) > 0){
                echo "<script>alert('You already commented on this movie.');window.location='review.php?movie_id=".$movie_id."';</script>";
            }else{
            ...
            $validateResult = mysqli_query($connect, $validateQuery);
            if(mysqli_num_rows($validateResult) > 0){
                echo "<script>alert('You already commented on this tv show.');window.location='review.php?tvshow_id=".$tvshow_id."';</script>";
            }else{
    ...
    if(isset($_POST['addWatchlistBtn'])){
        ...
        if ($movie_id) {
            $validateQuery = "SELECT * FROM watchlist WHERE user_id='$user_id' AND movie_id = '$movie_id'";
            $validateResult = mysqli_query($connect, $validateQuery);
    
            if ($validateQuery && mysqli_num_rows($validateResult)>0){
                echo"<script>
                        alert('Fail to add to watchlist. Movie has already existed in watchlist.');
                        window.location='review.php?movie_id=" . $movie_id . "';
                    </script>";
            }else{
                $query = "INSERT INTO watchlist(user_id, movie_id) VALUES ('$user_id', '$movie_id')";
            }
        } elseif ($tvshow_id) {
            $validateQuery = "SELECT * FROM watchlist WHERE user_id='$user_id' AND tvshow_id = '$tvshow_id'";
            $validateResult = mysqli_query($connect, $validateQuery);
    
            if ($validateQuery && mysqli_num_rows($validateResult)>0){
                echo"<script>
                        alert('Fail to add to watchlist. TV Show has already existed in watchlist.');
                        window.location='review.php?tvshow_id=" . $tvshow_id . "';
                    </script>";
            }else{
