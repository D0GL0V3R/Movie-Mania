...
<body>
    <div id = "main">
        <div id="faq">
            ...
            <form action="" method = "POST">
                <p>FAQ ID: </p>
                <select class = "textbox" id="txtdelete" name="txtdelete">
                    <option value="">Select an FAQ</option>
                    <?php
                    $getFaqSql = 'SELECT faq_id FROM faq';
                    $getFaqIdResult = mysqli_query($connect, $getFaqSql);
                    while ($row = mysqli_fetch_assoc($getFaqIdResult)) {
                        echo '<option value="' . htmlspecialchars($row['faq_id']) . '">' . htmlspecialchars($row['faq_id']) . '</option>';
                    }
                    ?>
                </select>
